...
class AdminListBurungActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mUserPref.getJenisUser().equals("penjual")){
            addBurung.visibility = View.VISIBLE
            addBurung.isEnabled = true
        }else{
            addBurung.visibility = View.INVISIBLE
            addBurung.isEnabled = false
        }
        addBurung.setOnClickListener {
            val i = Intent(this,AddBurungActivity::class.java)
            i.putExtra("tokoId",auth.currentUser?.uid)
            startActivity(i)
        }
        ...
        burungRef.get().addOnSuccessListener { result ->
            ...
            for (document in result){
                ...
                if (mUserPref.getJenisUser().equals("admin")){
                    ...
                    if (burung.idToko.equals(auth.currentUser?.uid)){
                        listBurung.add(burung)
                    }
                }
